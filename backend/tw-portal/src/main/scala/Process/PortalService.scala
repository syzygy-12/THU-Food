...
object PortalService {
  ...
  def handlePostRequest(client: Client[IO], req: Request[IO], serviceName: String, messageName: String): IO[Either[Throwable, Json]] = {
    for {
      bodyJson <- req.as[Json]  // 确保正确解析请求体中的 JSON 数据
      ...
      newUri <- IO.fromEither(Uri.fromString(s"http://${address(serviceName)}/api/$messageName"))  // 在 handlePostRequest 方法中，正确构建目标 URI
      _ <- IO.println(newUri)
  ...
  def sendRequest(client: Client[IO], uri: Uri, json: Json): IO[Either[Throwable, Json]] = {
    ...
    client.run(newReq).use { response =>
      ...
      if (response.status.isSuccess) {
        response.as[Json].attempt.map(_.left.map(err => new Exception(s"JSON parsing error: ${err.getMessage}", err)))  // 确保 sendRequest 方法正确发送请求并处理响应
      } else {
